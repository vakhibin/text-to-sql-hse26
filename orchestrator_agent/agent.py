...
_SYSTEM_PROMPT_HEADER = (
    ...
    "- Call at most one tool in a single assistant step. If the task requires "
    "multiple operations (for example modify SQL and then run it), call one "
    "tool, read its result, then call the next tool in the following step.\n"
    ...
    "- If the user refers to an earlier numbered item, option, query idea, or "
    "previous answer (for example 'implement item 2' or 'реализуй пункт 2'), "
    "resolve that reference from the conversation history before answering. "
    "Only ask for clarification if multiple previous lists could match.\n"
