...
```
Вопрос + ID базы данных
       |
  [1. Selector]        Векторный поиск (ChromaDB) + LLM-реранкер -> 3-5 релевантных таблиц
       |
  [2. Decomposer]      Классификация сложности + генерация подвопросов (CoT)
       |
  [3. Generator]        Ансамбль: N кандидатов асинхронно (primary + secondary модели, разнообразные few-shot)
       |
  [4. Exec Filter]     Валидация через SQLAlchemy -> отсев невалидных кандидатов
       |
  [5. Judge]            LLM-as-Judge -> выбор лучшего кандидата с обоснованием
       |
  [6. Refiner]          Исполнение -> при ошибке самокоррекция (макс. 3 итерации)
       |
   Итоговый SQL
```
