...
class SpiderSchemaLinker(BaseSchemaLinker):
    ...
    def _load_raw_schema(self) -> Dict:
        """Load raw schema from tables.json."""
        if self._raw_schema is not None:
            return self._raw_schema

        with open(self._tables_json_path, "r", encoding="utf-8") as f:
    ...
    def get_full_schema(self) -> List[TableInfo]:
        """Extract full schema from tables.json."""
        if self._schema_cache is not None:
            return self._schema_cache

        raw = self._load_raw_schema()
        ...
        col_idx_to_info: Dict[int, tuple] = {}  # col_idx -> (table_name, col_name)
        ...
        # Parse foreign keys: list of [from_col_idx, to_col_idx]
    ...
    def from_spider_dir(
        ...
        """
        Create linker from Spider dataset directory.
